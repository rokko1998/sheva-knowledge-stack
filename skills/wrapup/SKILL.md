...
5. Report the number written and queued. NotebookLM publishing is a separate step until the Google network issue is fixed. Never claim publication from a queued item. Do not call `notebooklm use` or create one source per session. Do not delete/replace NotebookLM sources without verifying ownership and authorization.
